...
//  ekfState(6) = ekfState(6) + dtIMU * gyro.z;	// yaw
  ...
  // Transform current state to quaternion.
  Quaternion<float> q_t = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  
  // Use IntegrateBodyRate to update the quaternion given body rates.
  Quaternion<float> q_t_bar = q_t.IntegrateBodyRate(gyro, dtIMU);
  
  // Use pitch, roll and yaw functions of quaternion class to find these values.
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  // Rotate accelerations from body to inertial frame, so we can add to state vector.
  V3F accelerationInInertialFrame = attitude.Rotate_BtoI(accel);
  ...
  predictedState(3) = curState(3) + accelerationInInertialFrame.x * dt;
  predictedState(4) = curState(4) + accelerationInInertialFrame.y * dt;
  predictedState(5) = curState(5) + (accelerationInInertialFrame.z - CONST_GRAVITY) * dt;
  
  // Do not update yaw as this is done in the IMU.
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  // Fill the row above the diagonal of the Jacobian.
  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
  
  // Fill the last column of the Jacobian.
  gPrime(3, 6) = (RbgPrime(0) * accel).sum() * dt;
  gPrime(4, 6) = (RbgPrime(1) * accel).sum()  * dt;
  gPrime(5, 6) = (RbgPrime(2) * accel).sum()  * dt;
  
  // Update the covariance to reflect additional uncertainty due to the predict step.
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  // Normalize the difference in yaw to -pi .. pi.
